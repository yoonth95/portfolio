      ...
      {propulsionData.map((itemPair, index, array) => {
        ...
        return Object.keys(contentItem).map((key) => (
          ...
          >
            ...
            {Array.isArray(contentItem[key]) ? (
              <ProjectImgSlide aspectRatio={aspectRatio}>
                {contentItem[key].map((imgSrc: string, imgIndex: number) => (
                  <img key={imgIndex} src={imgSrc} alt={`이미지 ${imgIndex}`} />
                ))}
              </ProjectImgSlide>
            ) : (
              <img src={contentItem[key] as string} alt={`이미지`} style={{ width: "100%", aspectRatio, objectFit: "contain" }} />
            )}
          </li>
        ));
      })}
